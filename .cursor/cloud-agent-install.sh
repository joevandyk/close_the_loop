...
# Cursor Cloud Agents run this from the repo root, but be defensive.
if root="$(git rev-parse --show-toplevel 2>/dev/null)"; then
  cd "$root"
fi

if ! command -v mix >/dev/null 2>&1; then
  echo "[cloud-agent] mix not found; skipping Elixir precompile"
  exit 0
fi
...
# Prefer `mix precompile` (requested), but fall back to `mix compile` if unavailable.
